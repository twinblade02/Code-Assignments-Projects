...
'''
# Count
count = dataset.make.value_counts().sort_index()

# dataframe for probabilities
make_prob = pd.DataFrame({'make': X['make'].unique(), 'make_P': Manufacturer})

check = pd.crosstab(index = X.make, columns = X.aspiration)
pd.crosstab(index = X.make, columns = X.aspiration)
...
for asp in X['aspiration'].unique():
    for make in X['make'].unique():
        ...
        if make_asp_count != 0:
            print("Prob(aspiration="+asp+"|make="+make+")"+" = " + str(round(100*make_asp_count/make_count,3)) + "%")
        else:
